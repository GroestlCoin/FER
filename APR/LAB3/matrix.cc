...
/*
method: inverse
output: Inverz matrice
*/
Matrix Matrix::inverse(void)
{
  if (this->getR() != this->getC())
    throw 8;
  
  Matrix ret(this->getR(), this->getC());
  Matrix P(this->getR(), this->getC());
  Matrix LUPMatrix(this->getR(), this->getC());
  ...
  {
    Matrix vecX(this->getR(), 1); //stupac matrice koja se trazi
    Matrix vecB(this->getR(), 1); //redak u jedinicnoj matrici
    Matrix vecY(this->getC(), 1); //medjukorak U*x
    
    
    vecB[i][0] = 1; //jedinica za jedinicnu matricu tj. konkretni stupa  
     
    vecY = LUPMatrix.subFG(vecB, P);
    
    vecX = LUPMatrix.subBG(vecY);  
